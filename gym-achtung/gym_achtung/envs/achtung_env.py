...
class AchtungEnv(gym.Env):
    ...
    number_of_players = 1
    def __init__(self):
        self.game = AchtungGame(FooEnv.number_of_players)
        self.players = [ self.game.get_player_by_id(id+1) for id in range(FooEnv.number_of_players)]
    ...
    def step(self, actions): # actions = the output of each network, the actions they want to do
        ...
        for i in range(1, FooEnv.number_of_players):
            chosen_network = np.random.choice(FooEnv.networks)
        ...
        reward = get_reward(game_over, players)
    ...
    def get_reward(self, game_over, players):
        if not game_over and player in players:
            return 6//len(players)
        elif game_over and player in players:
            return WIN_REWARD
